...
def explanation_linear(
    ...
    indices_active_elements = np.nonzero(instance)[
        1
    ]  # returns indices where feature value is non-zero (active elements)
    ...
    while not any(
        [
            (iteration >= max_iter),
            (np.size(scores_sorted) == 0),
            (nb_explanations >= max_explained),
            (time_max > 300),
            (stop != 0),
        ]
    ):
    ...
    return (
        explanation_set_adjusted[0:max_explained],
        number_active_elements,
        number_explanations,
        minimum_size_explanation,
        time_elapsed,
        explanations_score_change_adjusted[0:max_explained],
    )
