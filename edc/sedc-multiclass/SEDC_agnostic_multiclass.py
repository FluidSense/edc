...
class SEDC_Explainer_Multiclass(object):
    ...
    def explanation(self, instance):
        ...
        while (
            ...
            and (len(combinations_to_expand) != 0)
            and (len(new_combinations) != 0)
            and (time_max < (self.time_maximum))
        ):
            ...
            for (
                ...
                new_combinations
            ):  # verify each set in new_combinations if it is an explanation or not
                perturbed_instance = instance.copy()
                for feature_in_combination in combination:
                    perturbed_instance[:, feature_in_combination] = 0
                score_new = self.classifier_fn_multiclass(perturbed_instance)[
                    class_index
                ]

                if score_new[0] != np.max(
                    self.classifier_fn_multiclass(perturbed_instance)
                ):  # if class_index has no longer the top predicted score, an explanation is found.
            ...
            if len(scores_new_combinations_to_expand) != 0:
                index_combi_max = np.argmax(
                    score_predicted - scores_new_combinations_to_expand
                )  # best-first combination or feature is chosen.
            ...
            if (
                (len(new_combinations[0]) < max_length)
                and (expand == 1)
                and (stop == 0)
                and (nb_explanations < self.max_explained)
                and (len(new_combinations[0]) < self.max_features)
            ):

                print(
                    "length of new_combinations is %d features."
                    % len(new_combinations[0])
                )
                ...
                index_combi_max = np.argmax(
                    score_predicted - scores_combinations_to_expand
                )
                ...
                if (
                    (len(combinations) == 0)
                    or (nb_explanations >= self.max_explained)
                    or (len(combinations_to_expand) == len(new_combinations))
                ):
                    new_combinations = []
                    ...
                    while (
                        (len(new_combinations) == 0)
                        and (it < len(scores_combinations_to_expand2))
                        and (
                            (time_max + (time.time() - time_extra)) < self.time_maximum
                        )
                    ):
                        ...
                        if it != 0:
                            for index in indices:
                                scores_combinations_to_expand2[index] = (
                                    2 * score_predicted
                                )
                                # to make sure this index is never chosen again

                        index_combi_max = np.argmax(
                            score_predicted - scores_combinations_to_expand2
                        )  # best-first combi
                        ...
                        for combination in combinations_to_expand:
                            ...
                            for feature_in_combination in combination:
                                perturbed_instance[:, feature_in_combination] = 0
                            score_new = self.classifier_fn_multiclass(
                                perturbed_instance
                            )[class_index]
                        ...
                        print(
                            "score_predicted minus new_score is %f."
                            % (score_predicted - new_score)
                        )
        ...
        return (
            ...
            number_explanations,
            minimum_size_explanation,
            time_elapsed,
            explanations_score_change[0 : self.max_explained],
            iteration,
        )
